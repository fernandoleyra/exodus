...
export const SOURCES: Src[] = [
  ...
  },
  {
    ...
    licence: 'CC BY 4.0', licenceUrl: 'https://creativecommons.org/licenses/by/4.0/',
    locator: 'SP.POP.TOTL · SM.POP.TOTL · SM.POP.NETM · SL.UEM.TOTL.ZS · NY.GDP.PCAP.PP.KD · SH.MED.BEDS.ZS · SH.MED.PHYS.ZS · SL.EMP.TOTL.SP.ZS · SE.PRM.ENRL.TC.ZS · SM.POP.RHCR.EA/EO · SM.POP.ASYS.EA/EO · SM.POP.FDIP',
    url: 'https://databank.worldbank.org/source/world-development-indicators',
    vintage: 'annual, latest observation per country carried with its year',
    what: 'Country context, the service stocks behind the headroom model, and ten layers including five displacement series that were not wired before. Not all of it is observed: SP.POP.TOTL is UN World Population Prospects midyear estimates and SL.UEM.TOTL.ZS is a modelled ILO estimate, both badged as such, and the refugee and asylum-seeker series are UNHCR counts the Bank republishes.',
  ...
  },
  {
    ...
    locator: 'dissemination API 1.0 \u00b7 migr_resfirst, migr_resvalid, migr_eirtn \u00b7 migr_imm1ctz, migr_emi1ctz, migr_pop1ctz \u00b7 migr_asyappctzm, migr_asydcfstq, migr_asytpsm, migr_asytpfm, migr_asytpfq, migr_asywitfstq, migr_asypenctzm \u00b7 une_rt_a',
    url: 'https://ec.europa.eu/eurostat/web/main/help/copyright-notice',
    vintage: 'monthly to 2026-07, quarterly to 2026-Q2, annual to 2025 \u2014 each layer carries its own, shown on its chip',
    what: 'Twenty-five layers, from annual residence permits and population registers to monthly asylum applications and temporary protection \u2014 the freshest data in the application and the only near-complete bilateral cut of it. Reported counts throughout: what national statistical institutes actually recorded, which is the nearest thing to ground truth the modelled corridors can be checked against.',
    modified: 'Modified: JSON-stat decoded across every dimension, including ones not asked for \u2014 an undeclared dimension silently shifts every index. Aggregates are dropped on every axis, not just geo: the citizen axis carries TOTAL, EU27_2020 and EXT_EU27_2020 among the real members, and summing it naively returns about three times the truth. The newest period in a codelist is not the newest with data, so each layer is gated on reporter count against its trailing median and carries the real count. Observation flags are read alongside the values and are compound \u2014 ep, bep, ip and thirty-nine others \u2014 so testing for a bare p would miss most provisional figures. Armenia and Monaco are eliminated, because the Decision withholds commercial reuse for countries outside the EU, EFTA and the candidates, which is the action the copyright page itself prescribes. Also: the EU27_2020, EA20 and EA21 aggregates dropped; two-letter geo codes mapped to ISO-3166-1 alpha-3, including Eurostat\u2019s two departures from the standard (EL for Greece, UK for the United Kingdom).',
  },
  {
    id: 'imf-weo',
